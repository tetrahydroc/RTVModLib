...
func _ready():
	...
	if _repl.size() > 0:
		...
		if not _rtv_ready_done:
			super()
			_rtv_ready_done = true
	_lib._dispatch("catbox-_ready-post", [])
	_lib._dispatch_deferred("catbox-_ready-callback", [])
...
func _physics_process(delta: float):
	...
	if _repl.size() > 0:
		...
		super(delta)
	_lib._dispatch("catbox-_physics_process-post", [delta])
	_lib._dispatch_deferred("catbox-_physics_process-callback", [delta])
