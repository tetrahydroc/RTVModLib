extends "res://Scripts/Instrument.gd"

func _ready():
	var _lib = Engine.get_meta("RTVModLib", null)
	if !_lib:
		super()
		return
	_lib._dispatch("instrument-_ready-pre", [])
	var _repl = _lib._get_hooks("instrument-_ready")
	if _repl.size() > 0:
		var _prev_skip = _lib._skip_super
		_lib._skip_super = false
		_repl[0].callv([])
		var _did_skip = _lib._skip_super
		_lib._skip_super = _prev_skip
		if !_did_skip:
			super()
	else:
		super()
	_lib._dispatch("instrument-_ready-post", [])
	_lib._dispatch_deferred("instrument-_ready-callback", [])

func _physics_process(delta):
	var _lib = Engine.get_meta("RTVModLib", null)
	if !_lib:
		super(delta)
		return
	_lib._dispatch("instrument-_physics_process-pre", [delta])
	var _repl = _lib._get_hooks("instrument-_physics_process")
	if _repl.size() > 0:
		var _prev_skip = _lib._skip_super
		_lib._skip_super = false
		_repl[0].callv([delta])
		var _did_skip = _lib._skip_super
		_lib._skip_super = _prev_skip
		if !_did_skip:
			super(delta)
	else:
		super(delta)
	_lib._dispatch("instrument-_physics_process-post", [delta])
	_lib._dispatch_deferred("instrument-_physics_process-callback", [delta])

func Handling(delta):
	var _lib = Engine.get_meta("RTVModLib", null)
	if !_lib:
		super(delta)
		return
	_lib._dispatch("instrument-handling-pre", [delta])
	var _repl = _lib._get_hooks("instrument-handling")
	if _repl.size() > 0:
		var _prev_skip = _lib._skip_super
		_lib._skip_super = false
		_repl[0].callv([delta])
		var _did_skip = _lib._skip_super
		_lib._skip_super = _prev_skip
		if !_did_skip:
			super(delta)
	else:
		super(delta)
	_lib._dispatch("instrument-handling-post", [delta])
	_lib._dispatch_deferred("instrument-handling-callback", [delta])

func GetNextTrack():
	var _lib = Engine.get_meta("RTVModLib", null)
	if !_lib:
		return super()
	_lib._dispatch("instrument-getnexttrack-pre", [])
	var _result
	var _repl = _lib._get_hooks("instrument-getnexttrack")
	if _repl.size() > 0:
		var _prev_skip = _lib._skip_super
		_lib._skip_super = false
		var _replret = _repl[0].callv([])
		var _did_skip = _lib._skip_super
		_lib._skip_super = _prev_skip
		if _did_skip:
			_result = _replret
		else:
			_result = super()
	else:
		_result = super()
	_lib._dispatch("instrument-getnexttrack-post", [])
	_lib._dispatch_deferred("instrument-getnexttrack-callback", [])
	return _result

